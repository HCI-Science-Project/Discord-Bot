...
client.once("ready", () => {
    ...
    const commandFiles = readdirSync("./src/slash-commands").filter(file => file.endsWith(".js"))
    for (const file of commandFiles) {
        const command = require(`./slash-commands/${file}`)
        if (command.data) {
            commands?.create(command.data.toJSON())
                .catch(error => console.log("[SLASH COMMANDS]: An error occured loading a command"))
                .then(() => console.log(`[SLASH COMMANDS]: Successfully loaded command "${command.data.name}"`))
        }
    }
})
